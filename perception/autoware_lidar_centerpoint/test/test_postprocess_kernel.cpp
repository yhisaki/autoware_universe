...
void PostprocessKernelTest::SetUp()
{
  ...
  constexpr float point_feature_size{4.f};
  ...
  const std::vector<float> distance_bin_upper_limits{200.0f};
  const std::vector<float> score_thresholds{0.35f, 0.35f, 0.35f, 0.35f, 0.35f};
  ...
  config_ptr_ = std::make_unique<autoware::lidar_centerpoint::CenterPointConfig>(
    ...
    downsample_factor, encoder_in_feature_size, distance_bin_upper_limits, score_thresholds,
    circle_nms_dist_threshold, yaw_norm_thresholds, has_variance);

  postprocess_cuda_ptr_ = std::make_unique<PostProcessCUDA>(*config_ptr_, stream_);
...
class PostprocessKernelAnisotropicVarianceTest : public PostprocessKernelTest
{
  void SetUp() override
  {
    cudaStreamCreate(&stream_);
    ...
    constexpr std::size_t max_voxel_size{100000000};
    const std::vector<double> point_cloud_range{-76.8, -76.8, -4.0, 76.8, 76.8, 6.0};
    const std::vector<double> voxel_size{0.2, 0.4, 10.0};
    constexpr std::size_t downsample_factor{1};
    constexpr std::size_t encoder_in_feature_size{9};
    const std::vector<float> distance_bin_upper_limits{200.0f};
    const std::vector<float> score_thresholds{0.35f, 0.35f, 0.35f, 0.35f, 0.35f};
    constexpr float circle_nms_dist_threshold{0.5f};
    const std::vector<double> yaw_norm_thresholds{0.5, 0.5, 0.5};
    constexpr bool has_variance{true};
    ...
    std::vector<float> heatmap_host_vector(grid_xy_size * config_ptr_->class_size_, -1e6f);
    cudaMemcpy(
      head_out_heatmap_d_.get(), heatmap_host_vector.data(),
      grid_xy_size * config_ptr_->class_size_ * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemset(
      head_out_offset_d_.get(), 0,
      grid_xy_size * config_ptr_->head_out_offset_size_ * sizeof(float));
    cudaMemset(
      head_out_z_d_.get(), 0, grid_xy_size * config_ptr_->head_out_z_size_ * sizeof(float));
    cudaMemset(
      head_out_dim_d_.get(), 0, grid_xy_size * config_ptr_->head_out_dim_size_ * sizeof(float));
    cudaMemset(
      head_out_rot_d_.get(), 0, grid_xy_size * config_ptr_->head_out_rot_size_ * sizeof(float));
...
TEST_F(PostprocessKernelAnisotropicVarianceTest, PositionVarianceUsesRespectiveVoxelSizes)
{
  std::vector<Box3D> det_boxes3d;
  ...
  const auto grid_xy_size = config_ptr_->down_grid_size_x_ * config_ptr_->down_grid_size_y_;
  float score = 1.f;
  const float yaw_component = std::sin(M_PI_4);
  cudaMemcpy(
    &head_out_heatmap_d_[2 * grid_xy_size + index], &score, 1 * sizeof(float),
    cudaMemcpyHostToDevice);
  cudaMemcpy(
    &head_out_rot_d_[0 * grid_xy_size + index], &yaw_component, 1 * sizeof(float),
    cudaMemcpyHostToDevice);
  cudaMemcpy(
    &head_out_rot_d_[1 * grid_xy_size + index], &yaw_component, 1 * sizeof(float),
    cudaMemcpyHostToDevice);

  const float offset_log_variance = std::log(0.25f);
  cudaMemcpy(
    &head_out_offset_d_[2 * grid_xy_size + index], &offset_log_variance, 1 * sizeof(float),
    cudaMemcpyHostToDevice);
  cudaMemcpy(
    &head_out_offset_d_[3 * grid_xy_size + index], &offset_log_variance, 1 * sizeof(float),
    cudaMemcpyHostToDevice);
  ...
  const float scale = static_cast<float>(config_ptr_->downsample_factor_);
  const float expected_x_variance =
    config_ptr_->voxel_size_x_ * scale * std::exp(offset_log_variance);
  const float expected_y_variance =
    config_ptr_->voxel_size_y_ * scale * std::exp(offset_log_variance);

  EXPECT_NEAR(expected_x_variance, det_boxes3d[0].x_variance, 1e-4);
  EXPECT_NEAR(expected_y_variance, det_boxes3d[0].y_variance, 1e-4);
}
