...
{
  if (data.steps == 0 || data.batches == 0 || data.points == 0 || data.dims < 2) {
    return;
  }
  ...
  for (size_t step = 0; step < data.steps; ++step) {
    for (size_t batch = 0; batch < data.batches; ++batch) {
      for (size_t point = 0; point < data.points; ++point) {
  ...
  for (size_t step = 0; step < data.steps; ++step) {
    ...
    for (size_t batch = 0; batch < data.batches; ++batch) {
      ...
      points.reserve(static_cast<int>(data.points));
      for (size_t point = 0; point < data.points; ++point) {
        const auto index = (((step * data.batches + batch) * data.points + point) * data.dims);
...
void DenoisingStepChartWidget::ensureCharts(const size_t steps, const size_t batches, const int cols)
{
