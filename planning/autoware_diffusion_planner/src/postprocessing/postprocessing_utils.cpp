  ...
  const bool keep_current_state)
{
  ...
  const size_t output_trajectory_size =
    static_cast<size_t>(MAX_NUM_AGENTS) * OUTPUT_T * POSE_DIM;
  ...
  if (
    prediction.empty() ||
    (!has_current_state && prediction.size() % output_trajectory_size != 0)) {
      ...
      for (int64_t t = 0; t < output_t; ++t) {
        for (int64_t d = 0; d < POSE_DIM; ++d) {
          ...
          const size_t src_idx =
            ((static_cast<size_t>(b) * MAX_NUM_AGENTS + agent) *
  ...
  for (size_t step = 0; step < num_steps; ++step) {
    ...
    for (size_t batch = 0; batch < batch_size; ++batch) {
      ...
      msg.data.insert(
        ...
        denoising_predictions.begin() + static_cast<std::ptrdiff_t>(ego_offset + ego_trajectory_size));
    }
  }
