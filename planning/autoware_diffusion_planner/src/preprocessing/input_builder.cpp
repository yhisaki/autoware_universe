...
#include "autoware/diffusion_planner/preprocessing/items/ego_shape.hpp"
#include "autoware/diffusion_planner/preprocessing/items/goal_pose.hpp"
  ...
  const InputBuilderParams & params, std::vector<SelectedAgent> * selected_agents_out)
{
  ...
  // Neighbor agents on ego reference frame
  {
    const auto selected_agents = select_current_agents(
      frame_inputs.objects_history, frame_inputs.frame_time, map_to_ego_transform,
      MAX_NUM_NEIGHBORS);
    if (selected_agents_out != nullptr) {
      *selected_agents_out = selected_agents;
    }
