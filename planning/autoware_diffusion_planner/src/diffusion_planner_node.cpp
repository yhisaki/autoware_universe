...
: Node("diffusion_planner", options), generator_uuid_(autoware_utils_uuid::generate_uuid())
{
  ...
  // Service to enable/disable start guidance
  set_start_guidance_enabled_service_ = this->create_service<SetBool>(
    "~/service/set_start_guidance_enabled",
    std::bind(
      &DiffusionPlanner::on_set_start_guidance_enabled, this, std::placeholders::_1,
      std::placeholders::_2));
  set_stop_guidance_enabled_service_ = this->create_service<SetBool>(
    "~/service/set_stop_guidance_enabled",
    std::bind(
      &DiffusionPlanner::on_set_stop_guidance_enabled, this, std::placeholders::_1,
      std::placeholders::_2));
...
void DiffusionPlanner::set_up_params()
{
  ...
  params_.start_guidance_reference_distance_m =
    this->declare_parameter<double>(
      "guidance.start_guidance.reference_distance_m", 10.0);
  ...
  [[maybe_unused]] const std::vector<rclcpp::Parameter> & parameters)
{
  using autoware_utils::update_param;
  {
    ...
    update_param<std::string>(
      parameters, "model.multi_step_model.encoder_onnx_model_path",
      temp_params.encoder_model_path);
    update_param<std::string>(
      parameters, "model.multi_step_model.decoder_onnx_model_path",
      temp_params.decoder_model_path);
    update_param<std::string>(
    ...
    update_param<double>(
      parameters, "guidance.start_guidance.max_scale",
      temp_params.start_guidance_max_scale);
    update_param<double>(
  ...
  const SetBool::Request::SharedPtr request, const SetBool::Response::SharedPtr response)
{
  ...
  response->message = request->data ? "Start guidance enabled"
                                     : "Start guidance disabled";
}
...
void DiffusionPlanner::on_timer()
{
  ...
  try {
    planner_output = core_->create_planner_output(
      *inference_result, *frame_context, frame_time, generator_uuid_);
  } catch (const std::exception & e) {
